      ...
      <div className="grid gap-8 xl:grid-cols-[1fr_360px]">
            ...
            {history.length === 0 ? (
              ...
              history.slice(0, 5).map((entry) => (
                <div key={entry.id} className="flex flex-col gap-3 rounded-[24px] border border-white/[0.04] bg-white/[0.02] p-5 hover:border-white/[0.08] hover:bg-white/[0.04] transition-all">
                  ...
                  <p className="text-[13px] leading-relaxed text-aura-muted line-clamp-2 bg-black/20 rounded-[14px] p-3.5 border border-white/[0.02]">{normalizeTextContent(entry.result)}</p>
                </div>
              ))
            )}
            </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              <span className="text-[14px] font-semibold text-aura-text group-hover:text-white">View Managed Runtime</span>
