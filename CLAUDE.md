...
| Voice mode silent (no recording) | Three causes: (1) Electron blocks `getUserMedia` by default — need `setPermissionRequestHandler` in main/index.ts. (2) CSP blocks `wss://` — `connect-src https:` does NOT cover `wss:`, must add `wss:` explicitly. (3) `media-src` must include `blob:` for TTS audio. All three fixed as of 2026-03-30. |
| Voice mode shows no error, goes idle | VoicePanel now shows `voiceError` in caption area. If you see "Microphone permission denied" or "Voice not available", check the three causes above. |
