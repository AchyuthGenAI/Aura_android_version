# Aura Desktop — Developer Guide (AGENTS.md)
...
npm run package:mac          # unsigned universal .dmg (must run on macOS)
npm run package:mac:dir      # unsigned universal .app dir (must run on macOS)
...
- `extraResources`: copies the staged `build/openclaw-runtime/**` bundle → `resources/openclaw-src/`

Packaging preflight now stages OpenClaw into `build/openclaw-runtime/` before installer creation.

NSIS installer produces `Aura Desktop Setup x.x.x.exe` in `dist/`.
mac packaging produces an unsigned universal `.dmg` in `dist/` when run on macOS.
...
| Issue | Fix |
|-------|-----|
| `openclaw.mjs not found` | Run `npm run stage:openclaw-runtime` and verify `build/openclaw-runtime/openclaw.mjs` exists |
...
| macOS unsigned build blocked by Gatekeeper | Use the internal QA bypass path: right-click the app, choose Open, then confirm once in System Settings if needed. |
